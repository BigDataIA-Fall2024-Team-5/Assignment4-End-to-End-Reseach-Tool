...
def fetch_pdf_data_from_snowflake(**kwargs):
    ...
    cursor.close()
    conn.close()
    # Push records to XCom for use in downstream tasks
...
def process_and_chunk_pdf(pdf_link, title, id, **kwargs):
    ...
    # Push chunk texts to XCom for indexing in Pinecone
